...
int main() {
	...
	std::cout << "Задача 3.\nАдрес: ";
	std::string URL = "http://google.com/search/result";
...
std::string spaces_str(std::string str) { // реализация функции
	std::string result; // приобъявлении строки, она пустая
...
bool is_spam(std::string str) {
	...
	static const int spams_namber = 4;
	static std::string spams[spams_namber]{
		"100% free",
		"sales increase",
		"only today"
		"black friday"
	};
	for (int i = 0; i < spams_namber; i++)
